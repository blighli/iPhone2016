...
#import "Judge.h"
#import "PrintYearMonth.h"
#import "Days.h"

int main(int argc, const char * argv[]) {
    @autoreleasepool {
        int year,yue;
        PrintYearMonth *printMonthObj = [[PrintYearMonth alloc] init];
        PrintYearMonth *printYearObj = [[PrintYearMonth alloc] init];
        if (argc == 2) {
            //1.执行“cal”命令；
            //执行该命令时argc = 2;argv[1] = cal;argv[2] = null
            //计算当前年月；
            YearMonthNow *yearAndMonthNow = [[YearMonthNow alloc] init];
            ...
            Judge *judgeNumberRegex = [[Judge alloc] init];
